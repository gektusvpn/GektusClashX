...
class Build {
  ...
  static String get appName => "GektusClashX";

  static String get coreName => "GektusClashCore";
  ...
  // (GITHUB_REF_NAME, e.g. "v0.5.0-pre.1"), baked in via --dart-define=APP_VERSION.
  ...
  static final List<String> androidReleaseVersionArgs =
      _createAndroidReleaseVersionArgs();

  static List<String> _createAndroidReleaseVersionArgs() {
    final version = appVersion.replaceFirst(RegExp(r'^v'), '');
    if (version.isEmpty) return const [];

    final epoch = DateTime.utc(2026, 7, 18);
    const initialBuildNumber = 2026071801;
    final elapsed = DateTime.now().toUtc().difference(epoch);
    if (elapsed.isNegative) {
      throw StateError(
          'Android build timestamp predates the versionCode epoch');
    }
    final buildNumber = initialBuildNumber + elapsed.inSeconds ~/ 10;
    if (buildNumber > 2100000000) {
      throw StateError('Android versionCode exceeds the supported range');
    }
    return [
      '--build-name=$version',
  ...
  static buildHelper(Target target, String token, {Arch? arch}) async {
    ...
    final targetPath = join(
      outDir,
      target.name,
      "GektusClashHelperService${target.executableExtensionName}",
    );
  ...
  _buildWindowsApp({
    ...
    if (issTemplate.existsSync()) {
      final issContent = issTemplate
          .readAsStringSync()
          .replaceAll("{{APP_ID}}", "EE50BAE7-23E9-4BD7-9ACA-780E5B15BF97")
          .replaceAll("{{APP_VERSION}}", version)
          .replaceAll("{{DISPLAY_NAME}}", Build.appName)
          .replaceAll("{{PUBLISHER_NAME}}", "GektusVPN")
          .replaceAll(
              "{{PUBLISHER_URL}}", "https://github.com/gektusvpn/GektusClashX")
  ...
  _buildLinuxApp({
    ...
    if (arch == Arch.amd64) {
      ...
      File(specPath).writeAsStringSync(
        ...
        "%files\n"
        "/opt/$appName/*\n"
        "/usr/share/applications/com.gektus.clashx.desktop\n"
  ...
  _buildAndroidApp({
    ...
    await Build.exec(
      ...
      [
        ...
        "--dart-define=CORE_VERSION=$coreVersion",
        "--dart-define=APP_VERSION=${Build.appVersion}",
        ...Build.androidReleaseVersionArgs,
      ],
    );

    final outputDir = join(current, "build", "app", "outputs", "flutter-apk");
    Build.copyFile(
      join(outputDir, "app-release.apk"),
      join(Build.distPath, "${Build.appName}-android-universal.apk"),
    );
    print("✅ APK created in ${Build.distPath}");
  }
