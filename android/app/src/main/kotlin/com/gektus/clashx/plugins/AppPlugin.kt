package com.gektus.clashx.plugins
...
import com.gektus.clashx.GektusClashXApplication
import com.gektus.clashx.GlobalState
import com.gektus.clashx.R
import com.gektus.clashx.extensions.getActionIntent
import com.gektus.clashx.extensions.getBase64
import com.gektus.clashx.models.Package
...
// process-lifetime cache never showed apps (un)installed while GektusClashX ran.
...
class AppPlugin : FlutterPlugin, MethodChannel.MethodCallHandler, ActivityAware {
    ...
    private fun initShortcuts(toggle: String, start: String, stop: String) {
        val ctx = GektusClashXApplication.getAppContext()
    ...
    private fun tip(message: String?) {
        ...
        Toast.makeText(GektusClashXApplication.getAppContext(), message, Toast.LENGTH_LONG).show()
    }
                ...
                scope.launch {
                    ...
                    packageIcon.let {
                        ...
                        if (iconMap["default"] == null) {
                            iconMap["default"] =
                                GektusClashXApplication.getAppContext().packageManager?.defaultActivityIcon?.getBase64()
                        }
    ...
    private fun openFile(path: String) {
        ...
        val uri = FileProvider.getUriForFile(
            GektusClashXApplication.getAppContext(),
            "${GektusClashXApplication.getAppContext().packageName}.fileProvider",
            file
        )
        ...
        val resInfoList = GektusClashXApplication.getAppContext().packageManager.queryIntentActivities(
            intent, PackageManager.MATCH_DEFAULT_ONLY
        )

        for (resolveInfo in resInfoList) {
            val packageName = resolveInfo.activityInfo.packageName
            GektusClashXApplication.getAppContext().grantUriPermission(
                packageName,
                uri,
                flags
            )
    ...
    private fun updateExcludeFromRecents(value: Boolean?) {
        val am = getSystemService(GektusClashXApplication.getAppContext(), ActivityManager::class.java)
    ...
    private suspend fun getPackageIcon(packageName: String): String? {
        val packageManager = GektusClashXApplication.getAppContext().packageManager
    ...
    private fun getPackages(): List<Package> {
        ...
        packageManager?.getInstalledPackages(PackageManager.GET_META_DATA or PackageManager.GET_PERMISSIONS)
            ?.filter {
                it.packageName != GektusClashXApplication.getAppContext().packageName || it.packageName == "android"
    ...
    fun requestVpnPermission(callBack: (granted: Boolean) -> Unit) {
        val intent = VpnService.prepare(GektusClashXApplication.getAppContext())
        if (intent != null) {
            val activity = activityRef?.get()
            if (activity != null) {
    ...
    private fun isChinaPackage(packageName: String): Boolean {
        val packageManager = GektusClashXApplication.getAppContext().packageManager ?: return false
